...
import { HOST_WORKSPACES_DIR, WSL_WORKSPACES_RELATIVE_DIR } from './constants';
...
function normalizeIdentity(value: string): string {
  ...
  if (!normalized)
    throw new Error('A provider working directory is required to resolve a workspace.');
...
async function defaultMkdirWsl(distribution: string, value: string): Promise<void> {
  ...
  if (result.exitCode !== 0)
    throw new Error(result.stderr || `Unable to create WSL workspace: ${value}`);
}
...
function capabilityError(capability: Exclude<WslCapability, { status: 'ready' }>): Error {
  switch (capability.status) {
    ...
    case 'no-default-distribution':
      return new Error(
        `No default WSL distribution is configured. Available: ${capability.available.join(', ') || 'none'}.`,
      );
    case 'distribution-unavailable':
      return new Error(
        `WSL distribution '${capability.requested}' is unavailable. Available: ${capability.available.join(', ') || 'none'}.`,
      );
  }
}
...
export async function resolveWorkspaceContext(
  ...
  if (config.executionEnvironment === 'host') {
    ...
    try {
      ...
      throw new Error(
        `Unable to create Host workspace '${nativeRoot}': ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    return {
      workspaceId,
  ...
  try {
    ...
    throw new Error(
      `Unable to resolve WSL workspace home for '${distribution}': ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  if (!home.startsWith('/'))
    throw new Error('WSL home directory must be a Linux-native absolute path.');
  const nativeRoot = path.posix.join(home, WSL_WORKSPACES_RELATIVE_DIR, workspaceId);
  try {
    await mkdirWsl(distribution, nativeRoot);
  } catch (error) {
    throw new Error(
      `Unable to create WSL workspace '${nativeRoot}' in '${distribution}': ${error instanceof Error ? error.message : String(error)}`,
    );
  }
