...
- A `Skill Search Backend` setting now exists with `builtin`, `auto`, `qmd`, and `ck` options. Auto is the default so plugin-controlled qmd/ck can surface matches before slow built-in filesystem scans when available, with fallback to built-in behavior when unavailable. When selected and installed, `qmd` uses a plugin-managed QMD workspace/collection derived from configured skill roots, runs `collection add`/`update`/`embed` under isolated XDG state in plugin data, then queries that managed collection; `ck` uses CK `--jsonl --hybrid` search. QMD/CK executable fields and QMD collection config are hidden from the UI; standard `qmd`/`ck` are used under the hood with legacy saved executable overrides still honored. All external provider calls are plugin-controlled fixed-argument subprocesses with bounded timeouts, path-candidate parsing, and existing skill-root resolution. Models are explicitly instructed to call `list_skills` rather than raw `qmd`, `ck`, `grep`, or shell commands for skill discovery.
