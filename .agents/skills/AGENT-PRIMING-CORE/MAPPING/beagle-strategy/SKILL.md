...
description: Generate repo-grounded Beagle strategy artifacts and supporting workflow guidance.
analysis: |
  Use when the user wants a practical way to apply Beagle in a specific repo through browser-first artifacts, chosen-skill guidance, and lightweight execution planning.
  The skill should help turn repo context into a usable strategy bundle rather than a generic explanation.
  Use when the user wants to create a mental map for agents on beagle skill usage patterns and when to utilize them effectively in a practical way in a specific repo through browser-first artifacts, chosen-skill guidance, and lightweight execution planning.
  Also use proactively when you are about to render a complex ASCII table (4+ rows or 3+ columns) — present it as a styled HTML page instead.
license: MIT
compatibility: Requires a browser to view generated HTML artifacts.
metadata:
  author: AcidicSoil
  version: "0.0.1"
---
...
Beagle Strategy helps an agent turn repo context into a practical Beagle strategy bundle.

The goal is to produce something an agent can actually use later: a clear operating model, the Beagle skills that fit the repo, a lightweight plan for how work should flow, and a browser-first artifact bundle that can be reopened during future runs.

## Default behavior

When this skill is invoked without much extra direction, default to the current working repo and start from what is already available.

That usually means:

- use the active repo as the target
- generate the full strategy bundle first
- treat AI coding agents as the main audience
- prefer artifact generation over a long chat-only answer
- make the main deliverable a self-contained browser-friendly HTML artifact
- use local repo context, plans, docs, and structure as source material when available
- keep the chat reply short and focused on what was generated

Only ask follow-up questions when something important is missing and the result would otherwise be misleading or incomplete.

User overrides can still narrow the job, for example:

- a specific repo path
- a smaller output subset
- a different audience
- a different format

## What this skill is for

Use this skill when the user wants repo-grounded answers to questions like:

- which Beagle skills fit this codebase best
- what gates or review points should exist before and after implementation
- how different work types should move from intake to proof
- what artifacts later audits or follow-up runs should expect
...
- diff review
- project recap
- arbitrary comparison tables
- generic page generation unrelated to Beagle strategy

## What the strategy should include

A typical strategy bundle should cover:

1. the recommended operating model
2. installation guidance for agents only
3. agent gates or review checkpoints
4. scenario-based workflows
5. a skill-to-project mapping
6. chosen Beagle skills for the repo
7. an implementation plan with phases
8. a task list tied to that plan
9. anti-goals or boundaries
10. decisions, follow-ups, and next steps

The point is not to be exhaustive for its own sake. The point is to make later repo work easier to route, review, and revisit.

## Chosen skills and planning

The strategy should not stop at broad skill-group advice.

Where possible, choose the specific Beagle skills that make sense for the repo and explain:

- the exact skill id
- when it is useful
- which repo surfaces it applies to
- why it fits this repo
- what later work should be able to show if that skill was followed well

If the repo or user need it, the bundle can also include:

- a phased implementation plan
- a task list tied to the phases
- required Beagle skills per phase or task
- acceptance criteria that make later audits easier

This does not need to become a rigid process document. It should stay useful and readable.
...
| Command | Purpose |
|---|---|
| `generate-beagle-strategy` | Generate the overall Beagle strategy artifact bundle for a repo |
| `execute-active-task` | Work from the current approved task and keep artifacts aligned while the work progresses |
| `execute-next-task` | Select the next incomplete task and move the governed work forward |
| `audit-beagle-skill-compliance` | Review repo work against the chosen Beagle skills and note obvious gaps |
| `closeout-project-work` | Refresh artifacts, verify alignment, and leave the next handoff in a better state |

## Workflow

### 1. Gather repo context

Start from whatever useful context already exists:

- repo structure
- README or docs
- AGENTS guidance
- current plans or notes
- code surfaces that look important

### 2. Shape the strategy

Use that context to build a strategy bundle that stays grounded in the repo.

That usually includes:

- repo grounding
- the operating model
- installation guidance for agents only
- agent gates
- scenario workflows
- skill mapping
- chosen skills
- implementation phases and tasks
- anti-goals, decisions, and next steps

### 3. Render the artifacts

The default delivery model should stay browser-first.

The main output should usually be a polished self-contained HTML artifact, with supporting markdown and JSON files where they help execution, audit, or repo-local agent guidance.

Aim for pages that are clear, structured, and easy to reopen later. They should feel like working documents rather than decorative dashboards.

In practice that usually means:

- a clear title and short subtitle that explain what the artifact is for
- strong visual hierarchy so the important sections are easy to scan
- a small number of section types used consistently across the page
- simple navigation when the artifact is long enough to need it
- cards, lists, tables, and callouts only where they help understanding
- enough visual contrast to separate summary, detail, evidence, and follow-up sections
- browser-friendly layout and readable spacing in both wide and narrow windows

The goal is not to make every artifact flashy. The goal is to make it readable, grounded, and useful during future runs.

### 4. Keep later runs in mind

A good strategy bundle should still help on the next run.

If the workflow calls for it, keep the artifacts, repo map, and AGENTS reference content aligned so future agents can start from the generated bundle instead of reconstructing everything from memory.

## Output expectations

The primary deliverable should usually be a Beagle strategy artifact bundle for the target repo.

Supporting outputs may include:

- repo map markdown and JSON
- a self-contained HTML strategy artifact
- a self-contained HTML compliance runbook
- a self-contained HTML scenario workflow view
- markdown agent gates
- a skill mapping summary
- a markdown implementation plan
- next-step or decision notes

Only include outputs that directly help later work.

For token-saving local workflows, the agent may also use the scaffold script under `plugins/beagle-strategy/scripts/` to generate a fillable strategy starter pack first, then fill that in locally.

## Verification

Before wrapping up, check that:

- the skill still reads like a Beagle strategy tool rather than a generic explainer
- the output stays repo-grounded
- the chosen skills are concrete when the repo supports that level of specificity
- the plan and tasks are useful instead of ceremonial
- the primary HTML artifact remains browser-friendly and self-contained
- the supporting artifacts still help later execution rather than duplicating the same content in multiple places
...
### 2. Generate the visual strategy
...
- use that scenario only as a routing step
- identify the Beagle skill or skills that should be used for the current work
- collapse the strategy to one primary active task with a very small, clearly bounded work queue or set
- make the active task, task state, acceptance criteria, and next-task guidance easy to find

### 3. Execute from the active task

Once the strategy exists, the task becomes the main unit of work.

Execution should:

- start from the active task instead of broad scenario hopping
- force the Beagle skills mapped to that task before execution
- keep the task state honest as work moves to complete, in progress, or blocked
- update the supporting artifacts when task guidance, proof, or next-step instructions change

### 4. Resume the workflow cleanly

When work restarts, resume from the current task state.

That means:

- continue the in-progress task when possible
- keep blocked tasks clearly blocked until the blocker changes
- promote the next task only when the current one is complete or clearly blocked
- keep the strategy and supporting files aligned with repo reality
