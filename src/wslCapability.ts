...
export async function detectWslCapability(
  ...
  if (result.exitCode !== 0)
    return { status: 'wsl-unavailable', error: result.stderr.trim() || 'wsl.exe failed' };
  const available = parseDistributions(result.stdout);
  if (available.length === 0) return { status: 'no-distribution' };
  if (requested) {
    if (!available.includes(requested))
      return { status: 'distribution-unavailable', requested, available };
  ...
  if (!defaultDistribution || !available.includes(defaultDistribution))
    return { status: 'no-default-distribution', available };
