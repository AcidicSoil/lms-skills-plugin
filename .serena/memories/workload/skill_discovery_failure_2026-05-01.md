...
- The same export showed `searchBackend.requested: "qmd"` with `active: "builtin"` for an exact `python` match. That was not qmd fallback; exact-match short-circuit skipped enhanced search. Exact-match results now report `enhancedSkipped`, `enhancedSkippedReason`, and `resolutionStage` to avoid ambiguity.
