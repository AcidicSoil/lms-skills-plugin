...
export function recoveryError(code: RecoveryErrorCode, message: string): RecoveryError {
  const actions: Record<RecoveryErrorCode, RecoveryAction[]> = {
    ...
    "workspace-busy": [{ action: "wait-for-invocation", label: "Wait for the active workspace operation" }],
    ...
    "destructive-confirmation-required": [{ action: "confirm-destructive-action", label: "Confirm the destructive operation" }],
  };
