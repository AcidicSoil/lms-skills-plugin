    ...
    return [
      {
        ...
        hostPath:
          typeof profile.hostPath === 'string' && profile.hostPath.trim()
            ? profile.hostPath.trim()
            : undefined,
        wslPath:
          typeof profile.wslPath === 'string' && profile.wslPath.trim()
            ? profile.wslPath.trim()
            : undefined,
        ...
        repositoryIdentity:
          typeof profile.repositoryIdentity === 'string' && profile.repositoryIdentity.trim()
            ? profile.repositoryIdentity.trim()
            : undefined,
      },
    ];
  });
...
function normalizeApprovalHistory(value: unknown): ApprovalHistoryRecord[] {
  if (!Array.isArray(value)) return [];
  return value.filter((item): item is ApprovalHistoryRecord =>
    Boolean(
      item &&
      typeof item === 'object' &&
      typeof (item as ApprovalHistoryRecord).id === 'string' &&
      typeof (item as ApprovalHistoryRecord).workspaceId === 'string' &&
      typeof (item as ApprovalHistoryRecord).toolName === 'string' &&
      typeof (item as ApprovalHistoryRecord).timestamp === 'string',
    ),
  );
}
...
function normalizeChatSelections(value: unknown): Record<string, ChatWorkspaceSelection> {
  ...
  for (const [key, item] of Object.entries(value as Record<string, unknown>)) {
    ...
    if (
      (candidate.environment === 'host' || candidate.environment === 'wsl') &&
...
export function normalizePersistedSettings(parsed: Partial<PersistedSettings>): PersistedSettings {
  const skillsPaths =
    Array.isArray(parsed.skillsPaths) && parsed.skillsPaths.length > 0
      ? parsed.skillsPaths
          .map((value) =>
            parsed.executionEnvironment === 'wsl' ? value.trim() : expandHostSkillsPath(value),
          )
          .filter(Boolean)
  ...
  return {
    ...
    windowsShell:
      parsed.windowsShell === 'powershell' ||
      parsed.windowsShell === 'cmd' ||
      parsed.windowsShell === 'git-bash'
        ? parsed.windowsShell
        : DEFAULTS.windowsShell,
    executionEnvironment: parsed.executionEnvironment === 'wsl' ? 'wsl' : 'host',
    wslDistribution:
      typeof parsed.wslDistribution === 'string' && parsed.wslDistribution.trim()
        ? parsed.wslDistribution.trim()
        : undefined,
    workspaceProfiles: normalizeProfiles(parsed.workspaceProfiles),
    activeWorkspaceProfileId:
      typeof parsed.activeWorkspaceProfileId === 'string' && parsed.activeWorkspaceProfileId.trim()
        ? parsed.activeWorkspaceProfileId.trim()
        : undefined,
    hostWorkspacePath:
      typeof parsed.hostWorkspacePath === 'string' && parsed.hostWorkspacePath.trim()
        ? parsed.hostWorkspacePath.trim()
        : undefined,
    wslWorkspacePath:
      typeof parsed.wslWorkspacePath === 'string' && parsed.wslWorkspacePath.trim()
        ? parsed.wslWorkspacePath.trim()
        : undefined,
...
function loadSettings(): PersistedSettings {
  try {
    ...
    lastLoadDiagnostics = [
      { code: 'invalid-json', message: 'Settings could not be loaded; safe defaults were used.' },
    ];
    return { ...DEFAULTS };
  }
}
...
export function resolveEffectiveConfig(ctl: PluginController): EffectiveConfig {
  ...
  const maxSkillsInContext = (c.get('maxSkillsInContext') as number) ?? DEFAULTS.maxSkillsInContext;
  const rawPaths = ((c.get('skillsPath') as string | undefined) ?? '').trim();
  const shellPath = ((c.get('shellPath') as string | undefined) ?? '').trim();
  const windowsShell =
    (c.get('windowsShell') as 'powershell' | 'cmd' | 'git-bash' | undefined) ?? 'cmd';
  const executionEnvironment =
    (c.get('executionEnvironment') as 'host' | 'wsl' | undefined) === 'wsl' ? 'wsl' : 'host';
  const wslDistribution =
    ((c.get('wslDistribution') as string | undefined) ?? '').trim() || undefined;
...
export function getPlatformSafeSettings(platform: NodeJS.Platform = process.platform): {
