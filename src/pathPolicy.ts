...
export function resolveEnvironmentPath(input: string, environment: PathEnvironment, root?: string): PathValidationResult {
  ...
  const allowed = environment === "host-windows"
...
export function isContainedPath(root: string, target: string, options: ContainmentOptions = {}): boolean {
