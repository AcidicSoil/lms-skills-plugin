...
- `list_skills` search mode first returns fuzzy skill-name candidates from scanned metadata before trying enhanced qmd/ck or expensive full-text search, with a note telling the model to pick the intended exact skill and call `read_skill_file`. This ordering exists because an uploaded LM Studio failure export showed prompt-related queries timing out before returning obvious prompt skills.
- A `Skill Search Backend` setting now exists with `builtin`, `auto`, `qmd`, and `ck` options. Auto is the default so plugin-controlled qmd/ck can surface matches before slow built-in filesystem scans when available, with fallback to built-in behavior when unavailable. When selected and installed, `qmd` uses a plugin-managed QMD workspace/collection derived from configured skill roots, runs `collection add`/`update`/`embed` under isolated XDG state in plugin data, then queries that managed collection; `ck` uses CK `--jsonl --hybrid` search. QMD/CK executable fields are visible in plugin config again so users can provide executable names or absolute paths; standard `qmd`/`ck` remain the defaults and legacy saved executable overrides are still honored. QMD collection config stays managed by the plugin. All external provider calls are plugin-controlled fixed-argument subprocesses with bounded timeouts, path-candidate parsing, and existing skill-root resolution. Models are explicitly instructed to call `list_skills` rather than raw `qmd`, `ck`, `grep`, or shell commands for skill discovery.
- `read_skill_file` and `list_skill_files` no longer hard-stop on a wrong or partial skill name; they return ranked `suggestions` with exact names, scores, reasons, and paths so the model can retry with the best candidate. `read_skill_file` also handles root-relative skill directories and normalizes the common duplicated `SKILL.md` follow-up shape produced after `search_skill_roots`.
