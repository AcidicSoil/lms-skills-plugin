...
- A `Skill Search Backend` setting now exists with `builtin`, `auto`, `qmd`, and `ck` options. The implemented safe path remains built-in exact/fuzzy/route/full-text search; non-built-in selections are reported in `list_skills` results with built-in fallback metadata until plugin-controlled external adapters are implemented. Models are explicitly instructed to call `list_skills` rather than raw `qmd`, `ck`, `grep`, or shell commands for skill discovery.
