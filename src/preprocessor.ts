      ...
      return m.content
        .filter(
          (c): c is MessageContent =>
            typeof c === 'object' && c !== null && (c as MessageContent).type === 'text',
        )
...
function injectIntoMessage(message: MessageInput, injection: string): MessageInput {
  ...
  if (message !== null && typeof message === 'object') {
    ...
    if (Array.isArray(m.content)) {
      const first = m.content.findIndex(
        (c) => typeof c === 'object' && c !== null && (c as MessageContent).type === 'text',
      );
      if (first !== -1) {
...
function buildAutoInjectBlock(skills: SkillInfo[], limit: number): string {
  return [buildAutoInjectInstruction(), '', buildAvailableSkillsBlock(skills, limit)].join('\n');
}
 ...
 */
function buildExplicitActivationInstruction(resolved: string[], unresolved: string[]): string {
  const lines: string[] = [];
  ...
  lines.push('- The named skill is intentional; apply its instructions before anything else.');
  lines.push(
  ...
  if (unresolved.length > 0) {
    lines.push('');
    lines.push(`Unresolved skill reference(s): ${unresolved.map((n) => `/${n}`).join(', ')}`);
    lines.push(
 ...
 */
function buildSkillContextBlock(entries: Array<{ name: string; content: string }>): string {
  const inner = entries
    .map(({ name, content }) => [`<skill name="${name}">`, content.trim(), `</skill>`].join('\n'))
    .join('\n\n');
...
export async function promptPreprocessor(
  ...
  try {
    ...
    return injectIntoMessage(userMessage, buildAutoInjectBlock(skills, cfg.maxSkillsInContext));
  } catch (err) {
