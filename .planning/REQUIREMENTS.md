...
- [ ] **EXEC-01**: User can keep Host execution as the backward-compatible default.
- [ ] **EXEC-02**: Windows user can select WSL execution when WSL is available.
- [ ] **EXEC-03**: User can view and select from installed WSL distributions.
- [ ] **EXEC-04**: User receives an actionable error when WSL or the selected distribution is unavailable.
- [ ] **EXEC-05**: Existing persisted settings load without requiring manual migration.
...
- [ ] **PATH-01**: Relative project paths resolve from the active workspace root.
- [ ] **PATH-02**: Windows drive, Linux, and WSL UNC paths are classified and translated explicitly.
- [ ] **PATH-03**: Translation preserves spaces, Unicode, separator semantics, and drive-letter normalization.
- [ ] **PATH-04**: Project-scoped filesystem operations reject escape outside the workspace after canonicalization.
...
- [ ] **SAFE-01**: WSL invocation passes distribution, working directory, and command through argument-safe process APIs.
- [ ] **SAFE-02**: Host and WSL command execution enforce existing timeout and output limits.
- [ ] **SAFE-03**: Timeout handling attempts to terminate command descendants and reports incomplete termination.
- [ ] **TEST-01**: Automated tests cover execution settings, capability detection, path translation, and containment.
...
|-------------|-------|--------|
| EXEC-01..05 | Phase 1 | Pending |
| PATH-01..04 | Phase 1 | Pending |
| SAFE-01..03 | Phase 1 | Pending |
