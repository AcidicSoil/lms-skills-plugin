...
- Scanner/types now parse additional Claude-style skill frontmatter fields when present: `license`, `compatibility`, `metadata`, `paths`, `hooks`, and `shell`, in addition to existing fields like `allowed-tools`, `context`, `agent`, `model`, `effort`, `argument-hint`, and `arguments`.
