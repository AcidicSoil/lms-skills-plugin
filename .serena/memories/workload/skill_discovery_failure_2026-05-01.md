...
- A later agents export showed `search_skill_roots({ pattern: "agents" })` finding `agents/` subdirectories under parent skills but no `SKILL.md` entrypoints. The model guessed `list_skill_files(skill_name="reverse-engineer-rpi/agents")`, which is wrong because `agents` is a support directory inside a parent skill. `search_skill_roots` now annotates non-entrypoint matches with `parentSkill`, including `parentSkill.listSkillFilesArgs` and `parentSkill.readSkillFileArgs` when applicable, and the next step tells the model not to use matched subdirectory paths as `skill_name`.
- Another export showed the model receiving two timeout recovery payloads for `prompt engineering` and `agents`, then answering from general knowledge without attempting the required recovery calls. Timeout payloads now include `recoveryPlan`, `recoveryRequired: true`, `invalidFinalAnswerIf`, and explicit prohibition against substituting general knowledge for skill-catalog evidence.
- A follow-up export showed the model still ignored `recoveryPlan`: after `prompt engineering` timed out, it switched to the `agent` subtask instead of recovering the same subtask, then answered generically. Query timeout recovery now makes `search_skill_roots` the immediate required next call, moves list retry to optional second step, and explicitly says not to switch to another user subtask before same-subtask recovery.
