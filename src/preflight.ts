...
export function runPreflight(input: PreflightInput): PreflightResult {
  ...
  if (input.environment === "wsl") {
    ...
    if (!capability || capability.status === "pending" || capability.status === "unsupported-platform" || capability.status === "wsl-unavailable") {
      return { ok: false, error: recoveryError("environment-unavailable", "WSL is not currently available.") };
    }
    if (capability.status === "no-distribution" || capability.status === "no-default-distribution" || capability.status === "distribution-unavailable") {
      return { ok: false, error: recoveryError("distribution-missing", "The selected WSL distribution is unavailable.") };
    }
  }
