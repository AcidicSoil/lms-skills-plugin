...
import {
  ...
  validateOpaqueSessionReference,
...
export async function toolsProvider(
  ...
  let activeWorkspaceProfileId =
    restoredSelection?.profileId ?? initialSettings.activeWorkspaceProfileId;
  ...
  const getSkillStore = (): SkillStore => {
    if (!skillStore) {
      ...
      skillStore = factory(cfg, {
        execProgram: (program, args, options) =>
    ...
    if (!workspacePromise) {
      ...
      if (dependencies.resolveWorkspace) {
        ...
        const selected = activeWorkspaceProfileId
          ? (persisted.workspaceProfiles ?? []).find(
              (profile) => profile.id === activeWorkspaceProfileId,
            )
          : undefined;
        const selectedPath =
          currentConfig.executionEnvironment === 'wsl' ? selected?.wslPath : selected?.hostPath;
        if (selected && selectedPath && !selected.deleted && selected.enabled !== false) {
          workspacePromise = Promise.resolve({
            workspaceId: deriveWorkspaceId(
              selectedPath,
              currentConfig.executionEnvironment,
              currentConfig.wslDistribution,
            ),
            providerWorkingDirectory: selectedPath,
            executionEnvironment: currentConfig.executionEnvironment,
            ...(currentConfig.wslDistribution
              ? { wslDistribution: currentConfig.wslDistribution }
              : {}),
          ...
          context.executionEnvironment === 'wsl'
            ? dependencies.detectWsl
              ...
              : {
                  ...
                  distribution: context.wslDistribution ?? config.wslDistribution ?? 'default',
                  available: [context.wslDistribution ?? config.wslDistribution ?? 'default'],
                }
            : undefined;
  ...
  const getWorkspaceFs = (): Promise<WorkspaceFileSystem> => {
    if (!workspaceFsPromise)
      workspaceFsPromise = getBackend().then((backend) => backend.fileSystem);
  ...
  const resolveCommandDirectory = async (requested?: string): Promise<string> => {
    ...
    const pathApi =
      workspace.executionEnvironment === 'wsl'
        ? path.posix
        : /^[A-Za-z]:[\\/]/.test(workspace.nativeRoot)
          ? path.win32
          : path.posix;
  ...
  const detectWsl =
    dependencies.detectWsl ?? ((requested?: string) => detectWslCapability(requested));

  const getWorkspaceStatusTool = tool({
    name: 'get_workspace_status',
    description:
      'Inspect the active chat-scoped workspace selection and WSL capability without changing configuration.',
    parameters: {},
    implementation: async () => {
      const persisted = readSettings();
      const profileId = persisted.activeWorkspaceProfileId;
      const configuredPath =
        config.executionEnvironment === 'wsl'
          ? persisted.wslWorkspacePath
          : persisted.hostWorkspacePath;
      ...
      const workspace = deriveWorkspaceStatus(
        ...
        persisted.workspaceProfiles ?? [],
        {
          ...
          exists,
          configurationRequired: !resolvedPath && !profileId,
        },
      );
      const capability =
        config.executionEnvironment === 'wsl' ? await detectWsl(config.wslDistribution) : undefined;
      return {
        success: true,
        workspaceId,
        workspace,
        capability,
        globalDefaults: {
          hostPath: persisted.hostWorkspacePath,
          wslPath: persisted.wslWorkspacePath,
        },
      };
    },
  });
  ...
  const refreshWslCapabilityTool = tool({
    ...
    description:
      'Refresh WSL executable and distribution capability without changing the selected workspace or distribution.',
    parameters: {},
    implementation: async () => ({
      success: true,
      capability: await detectWsl(config.wslDistribution),
    }),
  });
  ...
  const configureHostWorkspaceTool = tool({
    ...
    description:
      'Configure the Host workspace path for the active plugin defaults. Only registered in Host mode.',
    ...
    implementation: async ({ path: workspacePath, profile_name }) => {
      ...
      const profiles = [
        ...(current.workspaceProfiles ?? []).filter((item) => item.id !== id),
        { id, name: profile_name?.trim() || 'Default workspace', hostPath: trimmed },
      ];
      const next = updateSettings({
        hostWorkspacePath: trimmed,
        activeWorkspaceProfileId: id,
        workspaceProfiles: profiles,
      });
      workspacePromise = undefined;
      backendPromise = undefined;
      workspaceFsPromise = undefined;
      activeCommandDirectory = undefined;
  ...
  const configureWslWorkspaceTool = tool({
    ...
    description:
      'Configure the WSL workspace path and optional distribution override. Only registered in WSL mode; omit distribution to use the system default.',
    parameters: {
      path: z.string().min(1),
    ...
    implementation: async ({ path: workspacePath, distribution, profile_name }) => {
      ...
      if (capability.status !== 'ready') {
        ...
        return {
          ...toRecoveryResponse(
            recoveryError(code, 'The requested WSL environment is unavailable.'),
          ),
          capability,
        };
      }
      ...
      const profiles = [
        ...(current.workspaceProfiles ?? []).filter((item) => item.id !== id),
        { id, name: profile_name?.trim() || 'Default workspace', wslPath: trimmed },
      ];
      const next = updateSettings({
        wslWorkspacePath: trimmed,
        wslDistribution: requested,
        activeWorkspaceProfileId: id,
        workspaceProfiles: profiles,
      });
      workspacePromise = undefined;
  ...
  const readSkillFileTool = tool({
    ...
    implementation: async ({ skill_name, file_path }, { status }) => {
      ...
      status(`Read ${Math.round(result.content.length / 1024)}KB from ${skill_name}`);
  ...
  const listSkillFilesTool = tool({
    ...
    parameters: {
      skill_name: z
        .string()
        .min(1)
        .describe("Skill directory name (e.g. 'docx') or an absolute path to a skill directory."),
      sub_path: z
        .string()
        .optional()
        .describe(
  ...
  const readFileTool = tool({
    name: 'read_file',
    description:
      'Read a UTF-8 file from the active per-chat workspace. Relative paths resolve from the workspace root; absolute paths must be native to the selected environment and remain contained.',
    parameters: {
      file_path: z
        .string()
        .min(1)
        .describe(
          'Workspace-relative path, or a contained absolute path native to the selected environment.',
        ),
    },
  ...
  const writeFileTool = tool({
    name: 'write_file',
    description:
      'Create or overwrite a UTF-8 file inside the active per-chat workspace. Prefer this over shell redirection.',
    parameters: {
      file_path: z
        .string()
        .min(1)
        .describe('Workspace-relative path, or a contained native absolute path.'),
  ...
  const patchFileTool = tool({
    name: 'patch_file',
    description:
      'Replace the first exact occurrence of a string in a file inside the active workspace.',
    parameters: {
      file_path: z
        .string()
        .min(1)
    ...
    implementation: async ({ file_path, search_string, replace_string }, { status }) => {
      ...
      try {
        ...
        ).patchFile(file_path, search_string, replace_string);
        return {
          success: true,
  ...
  const appendToFileTool = tool({
    name: 'append_to_file',
    description:
      'Append UTF-8 text to a file inside the active workspace, creating the file and parent directories when needed.',
    parameters: {
      file_path: z
        .string()
        .min(1)
  ...
  const createDirectoryTool = tool({
    ...
    description:
      'Create a directory and missing parents inside the active workspace. The operation is idempotent.',
    parameters: {
      dir_path: z
        .string()
        .min(1)
        .describe('Workspace-relative directory path, or a contained native absolute path.'),
    },
  ...
  const listDirectoryTool = tool({
    name: 'list_directory',
    description:
      'List files and directories inside the active workspace. Paths cannot escape the workspace root.',
    parameters: {
      dir_path: z
        .string()
        .optional()
        .describe('Workspace-relative directory path. Defaults to the workspace root.'),
      recursive: z
        .boolean()
        .optional()
        .describe('List descendants recursively, bounded by the workspace service.'),
    },
    implementation: async ({ dir_path = '.', recursive = false }, { status }) => {
      ...
      try {
        ...
        const formatted = formatDirEntries(
          result.entries,
          path.basename(result.path) || 'workspace',
        );
        return {
          success: true,
          dirPath: result.path,
  ...
  const deleteFileTool = tool({
    name: 'delete_file',
    description:
      'Permanently delete a file or directory inside the active workspace. The workspace root itself cannot be deleted.',
    parameters: {
      file_path: z
        .string()
        .min(1)
  ...
  const moveFileTool = tool({
    name: 'move_file',
    description:
      'Move a file or directory between contained locations in the active workspace. Existing destinations are not overwritten.',
    parameters: {
  ...
  const changeDirectoryTool = tool({
    ...
    description:
      'Change the active command directory inside the current Host/WSL workspace. The directory persists for subsequent run_command calls and cannot escape the workspace.',
    parameters: {
      dir_path: z
        .string()
        .min(1)
        .describe(
          'Contained directory path. Relative paths resolve from the current active command directory.',
        ),
    },
    implementation: async ({ dir_path }, { status }) => {
  ...
  const getCurrentDirectoryTool = tool({
    ...
    description:
      'Inspect the deterministic per-chat workspace used by project-scoped file and shell tools.',
    parameters: {},
  ...
  const runCommandTool = tool({
    name: 'run_command',
    description:
      'Execute a shell command in the active per-chat workspace and selected Host/WSL environment. An optional cwd must remain inside the same workspace.',
    parameters: {
      ...
      cwd: z
        ...
        .optional()
        .describe(
          'Optional contained directory. Relative paths resolve from the active command directory; defaults to the directory set by change_directory or the workspace root.',
        ),
      timeout_ms: z
        .number()
        .int()
        ...
        .optional()
        .describe(`Timeout in milliseconds. Defaults to ${EXEC_DEFAULT_TIMEOUT_MS}.`),
      env: z
        .record(z.string())
        .optional()
        .describe('Optional environment variables merged over the process environment.'),
    },
    implementation: async ({ command, cwd, timeout_ms, env }, { status }) => {
      ...
      try {
        ...
        const releaseInvocation = invocationRegistry.acquire({
          chatId: 'current-chat',
          profileId: activeWorkspaceProfileId,
          workspaceId: workspace.workspaceId,
        });
        let result;
        try {
          ...
          if (result.terminationIncomplete)
            invocationRegistry.markTerminationUnresolved(workspace.workspaceId);
        } finally {
          releaseInvocation();
        }
        return {
          ...
          workspaceId: workspace.workspaceId,
          workspaceRoot: workspace.nativeRoot,
          ...(result.terminationIncomplete !== undefined
            ? { terminationIncomplete: result.terminationIncomplete }
            : {}),
  ...
  const listWorkspacesTool = tool({
    ...
    implementation: async ({ query, cursor, limit, include_deleted }) => {
      const persisted = readSettings();
      const page = listWorkspaces(persisted.workspaceProfiles ?? [], {
        query,
        cursor,
        limit,
      ...
      return {
        success: true,
        workspacesEnabled: persisted.workspacesEnabled !== false,
  ...
  const addWorkspaceTool = tool({
    ...
    implementation: async ({ name, host_path, wsl_path, trusted, preferred, enabled }) => {
      const persisted = readSettings();
      const profiles = addWorkspace(persisted.workspaceProfiles ?? [], {
        name,
        hostPath: host_path,
        wslPath: wsl_path,
        trusted,
  ...
  const updateWorkspaceTool = tool({
    ...
    implementation: async ({
      ...
      const persisted = readSettings();
      const profiles = updateWorkspace(persisted.workspaceProfiles ?? [], profile_id, {
        ...(name !== undefined ? { name } : {}),
        ...(host_path !== undefined ? { hostPath: host_path } : {}),
        ...(wsl_path !== undefined ? { wslPath: wsl_path } : {}),
        ...(trusted !== undefined ? { trusted } : {}),
        ...(preferred !== undefined ? { preferred } : {}),
        ...(enabled !== undefined ? { enabled } : {}),
        ...(repository_identity !== undefined ? { repositoryIdentity: repository_identity } : {}),
      });
  ...
  const setWorkspacesEnabledTool = tool({
    ...
    implementation: async ({ enabled }) => ({
      success: true,
      workspacesEnabled: updateSettings({ workspacesEnabled: enabled }).workspacesEnabled,
    }),
  });
  ...
  const switchWorkspaceTool = tool({
    ...
    description:
      'Safely switch the active workspace for this chat after trust, identity, lifecycle, and invocation checks.',
    parameters: {
      profile_id: z.string().min(1),
      actual_repository_identity: z.string().optional(),
    },
    implementation: async ({ profile_id, actual_repository_identity }) => {
      const persisted = readSettings();
      if (persisted.workspacesEnabled === false)
        return toRecoveryResponse(
          recoveryError('workspace-disabled', 'Workspace functionality is disabled.'),
        );
      const profile = (persisted.workspaceProfiles ?? []).find((item) => item.id === profile_id);
      if (!profile)
        return toRecoveryResponse(
          recoveryError('workspace-invalid', 'Workspace profile was not found.'),
        );
      if (profile.deleted)
        return toRecoveryResponse(
          recoveryError('profile-deleted', 'Workspace profile is deleted.'),
        );
      if (profile.enabled === false)
        return toRecoveryResponse(
          recoveryError('workspace-disabled', 'Workspace profile is disabled.'),
        );
      if (!profile.trusted)
        return toRecoveryResponse(
          recoveryError('trust-required', 'Workspace profile is not trusted.'),
        );
      const candidatePath =
        config.executionEnvironment === 'wsl' ? profile.wslPath : profile.hostPath;
      if (!candidatePath)
        return toRecoveryResponse(
          recoveryError(
            'workspace-invalid',
            `No ${config.executionEnvironment} path is configured.`,
          ),
        );
      ...
      if (identity.status === 'mismatch')
        return toRecoveryResponse(
          recoveryError('identity-mismatch', 'Workspace repository identity does not match.'),
        );
      if (activeWorkspaceProfileId) {
        const current = (persisted.workspaceProfiles ?? []).find(
          (item) => item.id === activeWorkspaceProfileId,
        );
        const currentPath =
          config.executionEnvironment === 'wsl' ? current?.wslPath : current?.hostPath;
        if (currentPath) {
          const currentId = deriveWorkspaceId(
            currentPath,
          ...
          if (!guard.ok)
            return toRecoveryResponse(
              recoveryError(
                guard.reason === 'active' ? 'workspace-busy' : 'termination-unresolved',
                'The current workspace cannot be switched yet.',
              ),
            );
        }
      }
      ...
      updateSettings({
        activeWorkspaceProfileId: profile_id,
        chatWorkspaceSelections: chatState.set(chatId, config.executionEnvironment, profile_id),
      });
      workspacePromise = undefined;
      backendPromise = undefined;
      workspaceFsPromise = undefined;
  ...
  const deleteWorkspaceTool = tool({
    ...
    description:
      'Soft-delete or permanently delete a workspace profile when it is not active or busy.',
    ...
    implementation: async ({ profile_id, permanent }) => {
      ...
      const profilePath =
        config.executionEnvironment === 'wsl' ? profile.wslPath : profile.hostPath;
      if (profilePath) {
        const guard = invocationRegistry.canMutate(
          deriveWorkspaceId(profilePath, config.executionEnvironment, config.wslDistribution),
        );
        if (!guard.ok)
          return toRecoveryResponse(
            recoveryError(
      ...
      const profiles = permanent
        ...
        : softDeleteWorkspace(persisted.workspaceProfiles ?? [], profile_id);
      updateSettings({ workspaceProfiles: profiles });
  ...
  const restoreWorkspaceTool = tool({
    ...
    implementation: async ({ profile_id }) => {
      const persisted = readSettings();
      const profiles = restoreWorkspace(persisted.workspaceProfiles ?? [], profile_id);
      updateSettings({ workspaceProfiles: profiles });
      return { success: true, profile: profiles.find((item) => item.id === profile_id) };
    },
  });
  ...
  const getSessionCapabilityTool = tool({
    ...
    parameters: {},
    implementation: async () => {
      const capability = dependencies.sessionCapability ?? unsupportedSessionCapability;
      return capability.status === 'supported'
        ? { success: true, status: 'supported' }
        : { success: true, status: 'unsupported', reason: capability.reason };
    },
  });
  const resumeSessionTool = tool({
    name: 'resume_session',
    description:
      'Resume an opaque host session reference when a stable host capability is available.',
    parameters: { session_ref: z.string().min(1) },
    implementation: async ({ session_ref }) => {
      const capability = dependencies.sessionCapability ?? unsupportedSessionCapability;
      if (capability.status !== 'supported')
        return toRecoveryResponse(recoveryError('environment-unavailable', capability.reason));
      try {
        return {
          success: true,
          ...(await capability.resume(validateOpaqueSessionReference(session_ref))),
        };
      } catch (error) {
        return failure(error);
      }
  ...
  const listWorkspaceApprovalsTool = tool({
    ...
    implementation: async ({ workspace_id }) => {
      const persisted = readSettings();
      const store = new ApprovalHistoryStore(persisted.approvalHistory ?? []);
      return { success: true, items: store.list(workspace_id) };
    },
  });
  const revokeWorkspaceApprovalTool = tool({
    ...
    implementation: async ({ approval_id }) => {
      const persisted = readSettings();
      const store = new ApprovalHistoryStore(persisted.approvalHistory ?? []);
      updateSettings({ approvalHistory: store.revoke(approval_id) });
      return { success: true, approvalId: approval_id };
    },
  });
  const clearWorkspaceApprovalsTool = tool({
    ...
    implementation: async ({ workspace_id }) => {
      const persisted = readSettings();
      const store = new ApprovalHistoryStore(persisted.approvalHistory ?? []);
      updateSettings({ approvalHistory: store.clearWorkspace(workspace_id) });
  ...
  return [
    ...
    resumeSessionTool,
    ...(config.executionEnvironment === 'wsl'
      ? [configureWslWorkspaceTool, refreshWslCapabilityTool]
      : [configureHostWorkspaceTool]),
  ];
}
