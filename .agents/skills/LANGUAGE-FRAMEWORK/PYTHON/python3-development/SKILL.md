...
description: 'Use this skill for Python project work: app/package design, script writing or editing, CI tools, stdlib-only portable scripts, running scripts/tests, debugging Python or python3 failures, fixing pre-commit/lint issues in Python files, reviewing Python code, and writing or updating tests for scripts, packages, or apps. Also use it when Typer, Rich, or Python CLI apps are involved, or when editing Python code in a git repo.
<hint>Provides the user’s preferred workflows for TDD, feature work, refactoring, debugging, and review with modern Python 3.11+ patterns; favored modules; working pyproject.toml examples; lint/format configs and troubleshooting; known-error fixes; and preferred project layouts.</hint>'
version: "1.1.0"
